...
const Register = () => {
  ...
  const handleGoogleSuccess = async (tokenResponse) => {
      try {
          const res = await axios.post(import.meta.env.VITE_API_URL + '/api/v1/auth/google', {
    ...
    <div className="min-h-[calc(100vh-70px)] bg-[#F8F9FA] dark:bg-[#0B1120] transition-colors duration-300 flex flex-col justify-center py-12 sm:px-6 lg:px-8 mb-10">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          ...
          <div className="mt-8">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-3 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 font-medium">{t('or_sign_up_with')}</span>
              </div>
            </div>
        ...
        <p className="mt-8 text-center text-sm text-gray-600 dark:text-gray-400">
          ...
          <Link to="/login" className="font-bold text-[#FF4D20] hover:text-orange-600 hover:underline">
            {t('sign_in')}
          </Link>
        </p>
