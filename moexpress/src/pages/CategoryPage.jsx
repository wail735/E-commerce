...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    setActiveSub(subCategories[slug]?.[0] || "All");
    setSelectedBrands([]);
    setMaxPrice(1000);
    setMinPrice(1);
    setMinRating(0);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/products?category=${slug}`);
                ...
                <div className="flex items-center justify-between mb-5 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 px-4 py-2.5 shadow-sm">
                  <div className="flex items-center gap-4">
                    <span className="text-[13px] text-gray-500 dark:text-gray-400">
                      {t('showing')} <strong className="text-gray-800 dark:text-gray-200">{filtered.length}</strong> {t('of')} {products.length} {t('products')}
                    </span>
                    <button
