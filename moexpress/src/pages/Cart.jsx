            ...
            <div className="space-y-4">
              {cartItems.map((item) => (
                <div key={item.id} className="bg-white dark:bg-gray-800/50 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-gray-700 flex gap-4 relative shadow-sm hover:shadow-md transition-shadow">
                  ...
                  {/* Image */}
                  <Link to={`/product/${item.slug}`} className="w-24 h-24 sm:w-28 sm:h-28 bg-[#F7F7F8] rounded-xl flex items-center justify-center overflow-hidden shrink-0">
                    <img src={item.image} alt={t(`prod_${item.slug.replace(/-/g, '_')}`)} className="w-full h-full object-contain mix-blend-multiply hover:scale-105 transition-transform p-2" />
                  </Link>
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <Link to={`/product/${item.slug}`} className="text-[15px] font-bold text-gray-900 dark:text-gray-100 hover:text-[#FF4D20] dark:hover:text-[#FF4D20] transition-colors mb-0.5 line-clamp-1">
                          {t(`prod_${item.slug.replace(/-/g, '_')}`)}
                        </Link>
